...
public class FilePrinter
{
   ...
   @Autowired
   public FilePrinter(@Value("${local.fonts}") String fontLocation,
                      @Value("${report.folder}") String fileFolderLocation)
   {
      try
      {
   ...
   public File print(Map<Constructor, List<Order>> constructorsWithOrders)
   {
      try
      {
         ...
         File file = new File(fileFolderLocation + File.separator + "Report_" + period.getFirst().format(formatter)
               + "_" + period.getSecond().format(formatter) + ".pdf");
   ...
   private void setContent(Document document, Map<Constructor, List<Order>> constructorsWithOrders)
         throws DocumentException
   {
      ...
      Chunk chunk4 = new Chunk(
            "за период с " + period.getFirst().format(formatter) + " по " + period.getSecond().format(formatter));
